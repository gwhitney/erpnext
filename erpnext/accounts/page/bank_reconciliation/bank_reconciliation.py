...
def reconcile(bank_transaction, payment_doctype, payment_name):
	...
	if payment_entry.clearance_date:
...
def check_matching_amount(bank_account, company, transaction):
	...
	if transaction.currency == company_currency:
		...
		currency_filter = [[currency_field, "=", transaction.currency]]
...
def get_matching_descriptions_data(company, transaction):
	...
	for key, value in iteritems(links):
		...
		if key == "Sales Invoice":
			data.extend(frappe.get_all("Sales Invoice", filters=[["name", "in", value]], fields=["'Sales Invoice' as doctype", "posting_date", "customer_name as party", "paid_amount as pymt_amount", "currency"]))
		if key == "Purchase Invoice":
			data.extend(frappe.get_all("Purchase Invoice", filters=[["name", "in", value]], fields=["'Purchase Invoice' as doctype", "posting_date", "supplier_name as party", "paid_amount as pymt_amount", "currency"]))
		if key == "Expense Claim":
			expense_claims = frappe.get_all("Expense Claim", filters=[["name", "in", value]], fields=["'Expense Claim' as doctype", "posting_date", "employee_name as party", "total_amount_reimbursed as pymt_amount"])
